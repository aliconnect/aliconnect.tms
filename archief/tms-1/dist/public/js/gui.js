...
(function() {
  ...
  .on('ready', async event => {
    ...
    function tabSelect(el) {
      $.elSignalsBevCont.elem.style.display = 'none';
    ...
    $(document.body).class('col gui').append(
      $('div').class("row bartop").id("elBartop").append(
        $('div').id('btnbar'),
        $.elemTitle = $('span'),
        $.timeElem = $('span').style('margin-left:auto'),
      ...
      $('div').class("row aco").append(
        $.elTabsLeftControl = $('div').class("row tabsControl").id("elTabsLeftControl"),
        $('div').class("row aco tabsleft").append(
          $.elPanelTree = $('ul').class("col panel").id("elPanelTree"),
          $.elPanel = $('ul').class("col panel aco").id("elPanel"),
        ),
        $('div').class("col panelright").append(
          $('div').class("col aco signals").append(
            ...
            $.elSignalsBevCont = $('div').id("elSignalsBevCont").class("col aco").append(
          ...
          $.tabsBottom = $('div').id("elTabsBottom").class("col").append(
            ...
            $.elTabSystem = $('ul').class('col tab system').style('display:none;'),
          ),
          $.tabsBottom.control = $('div').class("row tabsControl").id("elTabsBottomControl").append(
            $('a').text('Meldingen').attr('selected','').on('click', function(event) {
              tabSelect(this);
              $.elSignalsBevCont.elem.style.display = '';
            }),
            $.buttonVerkeer = $('a').class('verkeer').text('Verkeer').attr('selected','').on('click', function(event) {
              tabSelect(this);
            }),
          ),
        ),
    ...
    if ($().body) {
      ...
      $.elemTitle.text('LOADING CONFIGURATION DATA');
    ...
    $.elemTitle.text($().info ? $().info.title + ' ' + $().info.description : '');

    $.buttonVerkeer.elem.click()
    Object.entries(menu).forEach(([name1,topmenuitem]) => $('li').parent($.elPanelTree).append(
      ...
      $('ul').class('col').append(
        Object.entries(topmenuitem.children).map(([name2,menuitem]) => {
          ...
          menuitem.name = name2;
          menuitem.elUL = $('ul').parent($.elPanel).class('col');
          return $('li').append(
    ...
    setInterval(event => $(document.body).attr('blink', $.blink^= 1), 1000);
    setInterval(event => $.timeElem.text(new Date().toLocaleTimeString()), 1000);
    ...
    items.filter(item => item.schemaName === 'Verkeersbuis').forEach(item => {
      // ($.elTabsLeftControlBuis.items = $.elTabsLeftControlBuis.items || []).push(item);
