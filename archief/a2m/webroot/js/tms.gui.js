...
aim.extend(GUITMS = {
	on: {
		init: function() {
			aim.api.request({ path: `/System(${aim.config.aim.aud})?child=20&select=*&src=10&link=10&refby=10` }, function(event) {
				//console.debug("AAAAAAAAAAAAABBBBB",aim.config.aud, aim.ref[aim.config.aud]);
				//return;
				aim.createBody(aim.ref[aim.config.aim.aud]);
			});
		}
	},
		...
		(function build(item, level) {
			...
			if (item.children && item.children.length) {
				...
				item.children.forEach(function (child) {
					...
					if (child.gui) {
						if (child.gui.create) child.gui.create.call(child);
